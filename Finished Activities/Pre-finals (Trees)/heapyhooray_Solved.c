...
void heapify(MinHeap* pq, int i) {
    ...
    int minimum = -1; 
    while (i < pq->size && minimum != i){
      minimum = i; 
      if (LC < pq->size && pq->heap[LC].duration < pq->heap[i].duration){
        i = LC;
      }
      if (RC < pq->size && pq->heap[RC].duration < pq->heap[i].duration){
        i = RC;
      }

      if (i != minimum){
        Task temp = pq->heap[minimum];
        pq->heap[minimum] = pq->heap[i];
        pq->heap[i] = temp;
        LC = (2 * i) + 1; 
        RC = LC + 1;
      }
