...
void insertAuto(dictionary D, char elem){
    ...
    if (D->elems[ELEM_NDX].data != DELETED && D->elems[ELEM_NDX].data != EMPTY){
      if (D->elems[ELEM_NDX].data != elem){
        ...
        if (*trav == -1){
          int temp = allocSpace(&D->VH);
          if (temp == -1) {
            printf("[INSERT] VH OUT OF SPACE!\n");
          } else {
            D->VH.synonyms[temp].link = -1;
            D->VH.synonyms[temp].data = elem;
            *trav = temp;
          }
        } else {
