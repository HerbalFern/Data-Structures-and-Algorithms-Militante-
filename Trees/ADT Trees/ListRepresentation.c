...
void addChild(Tree *T, Node parent_index, Node child_index) {
    // TODO
    // apparently insert first according to wayne
    ChildNode* temp = calloc(1, sizeof(*temp)); 
    temp->next = T->children[parent_index];
    temp->childIndex = child_index; 
    T->children[parent_index] = temp;
}
...
Node parent(Node n, const Tree *T) {
    ...
    ChildNode* trav;
    for (i = 0; i < MAX_NODES; i++){
        for (trav = T->children[i]; trav != NULL; trav = trav->next){
            if (trav->childIndex == n) return i;
        }      
    }
    return INVALID_NODE; 
}
...
Node left(const Tree *T) {
    // TODO
    if (T->rootIndex == INVALID_NODE) return INVALID_NODE;
    ChildNode* trav;
    int i, left = 99999;
    for (i = 0; i < MAX_NODES; i++){
        for (trav = T->children[i]; trav != NULL; trav = trav->next){
            if (trav->childIndex < left && T->children[trav->childIndex] == NULL) left = trav->childIndex; 
        }
    }
    return left; 
}
...
Node right(const Tree *T) {
    ...
    for (i = 0; i < MAX_NODES; i++){
        for (trav = T->children[i]; trav != NULL; trav = trav->next){
            if (trav->childIndex > right && T->children[trav->childIndex] == NULL) right = trav->childIndex; 
        }
    }
    return right; 
}
