...
Node left(Tree T) {
    ...
    for (i = 0; i < MAX_NODES; i++){
        ...
        if (flag == 0 && i < left) left = i;
    }
    return left != 99999 ? left : INVALID_NODE;
...
Node right(Tree T) {
    ...
    for (i = 0; i < MAX_NODES; i++){
        ...
        if (flag == 0 && i > right) right = i;
    }
    return right != -9999 ? right : INVALID_NODE;
}
